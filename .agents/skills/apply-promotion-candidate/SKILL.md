...
`docs/quality/promotion-candidates/` и превратить её в проверяемое изменение
...
1. Найти корень `new-project-rules`: каталог должен содержать `AGENTS.md`,
   ...
   `docs/quality/PROMOTION_CANDIDATES.md`, каталог candidate files и
   соответствующие шаблоны/skills.
...
2. Если кандидат затрагивает общий rule layer, проверить, не нужно ли также
   ...
   правилам проекта.
3. Сохранить traceability в файле выбранного кандидата: обновить статус на
   `implemented`, записать `implemented_commit` и итоговый target. Общий index
   не менять только ради статуса одной записи.

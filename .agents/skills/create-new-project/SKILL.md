...
2. Действовать только с согласия пользователя (opt-in); ничего не устанавливать
   ...
   - база не найдена → предложить установить: `git clone
     git@github.com:NightMadMax/best-practices.git "Best Practices"` рядом с
     проектом в общий vault (целевую папку задавать явно, чтобы совпадала со
     ссылкой `../Best Practices` в правиле нового проекта).

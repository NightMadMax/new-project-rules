...
2. Если профиль не указан, использовать `software`. Допустимые профили:
   ...
   Capability независима от профиля. Поддерживаются `jira-confluence`, `1c` и
   `transcribe`: при выборе добавить `-Capability <имя>` в PowerShell bootstrap
   или четвёртым аргументом в shell bootstrap. Для `jira-confluence`
   одновременно выбрать одноимённый стек Best Practices. `1c` и `transcribe`
   требуют профиля не ниже `operated` и `software` соответственно — минимум
   объявлен в `config/capability-core.tsv`.
...
3. Убедиться, что папка проекта находится внутри общего Obsidian vault, но сама
   не является vault.
4. Проверить instruction sources в новом процессе. Сначала найти запускаемый
   CLI: `sh scripts/check-cli.sh codex` (в PowerShell —
   `.\scripts\check-cli.ps1 codex`) и взять путь из отчёта — вызов по имени
   даёт ложный ответ в обе стороны. Затем:
   `<путь>/codex --cd <destination> --ask-for-approval never exec "Summarize the current instructions."`.
   Статус `skipped` — это пропуск проверки с названной причиной, а не отказ
   создания проекта.

...
7. Проверить read-only состояние global policy и показать secret-safe план:
   ...
   установлены. Не добавлять markers вручную и не изменять active file: его
   миграция требует отдельного подтверждаемого workflow.
8. Проверить, что Obsidian установлен и родительская рабочая папка открыта как

...
3. Запустить read-only проверку окружения в выбранном режиме:
   - macOS/Linux: `./scripts/check-environment.sh <mode>`;
   - Windows: `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\check-environment.ps1 -AgentMode <mode>`,
...
1. Повторно запустить проверку окружения в том же agent mode.

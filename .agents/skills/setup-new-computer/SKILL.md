...
3. После изменения instruction-файлов запустить новый процесс Codex с
   `codex --ask-for-approval never "Summarize the current instructions."`; для
   scoped rules использовать `codex --cd <directory>` и проверить приоритет
...
4. Проверить `gh auth status`, Git identity, credential helper, remote и чистоту
   clone правил.
5. Сообщить отдельно: что настроено, что уже было настроено и какие ручные шаги

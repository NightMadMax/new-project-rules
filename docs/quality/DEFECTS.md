...
|---|---|---|---|---|---|
| 56 | Команда проверки instruction sources запускала интерактивный Codex TUI | 2026-07-07 | 2026-07-07 | (в этой ветке) | В аудит и производные инструкции попала форма `codex ... "prompt"` без subcommand; в non-TTY она завершалась ошибкой `TERM is set to "dumb"`. Во всех исполняемых guides/templates/skills добавлен `codex exec`, после чего проверка выполняется отдельным non-interactive процессом. |
| 55 | Global policy и шаблоны закрепляли неподтверждённые ограничения AGENTS.md | 2026-07-07 | 2026-07-07 | (в этой ветке) | Глобальный, проектный и template-слои копировали community-рекомендации как технические факты: лимиты 150/300 строк, recite-back, запрет scoped override и неверную причину mid-session. Аудит выявил расхождение с документацией; global policy сокращена, проектные процессы оставлены в template/skills, validator и policy contract очищены от вымышленных порогов. |

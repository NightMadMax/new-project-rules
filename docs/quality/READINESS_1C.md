...
| 22 | 13 ролей и 13 upstream-команд в согласованных проекциях | частично | `scripts/test-one-c-scaffold.py`, №197 |
...
Из 31 критерия: `тест` — 12, `частично` — 7, `Windows` — 0, `не выполнено` — 11, `отложено` — 1.
...
- **№197 — нет command-owner workflows** (критерий 22 наполовину). Доставка
  выполнена (№172 закрыт, PR #46): 13 ролей стоят в обоих клиентах и проверяются
  тестом, OpenSpec с четырьмя workflow закрыл критерий 25. `develop-1c` написан
  и доставлен вместе с восемью references. Тринадцати команд по-прежнему нет:
  `evolve-1c-rules` и `manage-1c-mcp` отсутствуют, `deploy-1c-source` и
  `deploy-and-test-1c` объявлены без исполнителя (№165).

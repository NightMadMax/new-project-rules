...
Критерий 7 — `частично`, и это исправление задним числом. 2026-08-02 он был
переведён сразу в `тест`: решение 1.16 пересмотрено, пересобирать обработку не
нужно, три собранные поставляются вместе с профилями. Управляемая половина
действительно закрыта — состояние переключателя читается вызовом и проверяется
исполнителем session lock. Обычная — нет: путь в `ATTR_STARTUP_OPTION`
указывает на `<PROJECT_ROOT>`, которого не подставляет ни bootstrap, ни EDT, и
профиль в поставленном виде обработку не запускает (№252). Проверка, принявшая
это за готовность, смотрела на наличие `/Execute` и не смотрела, куда он ведёт;
теперь `doctor-1c` сверяет путь с проектом и сообщает `SKIP`. Статус вернётся в
`тест`, когда путь будет проверен на машине с EDT.

Половин больше нет — кроме названной выше. №175 закрыт поставкой карточки базы

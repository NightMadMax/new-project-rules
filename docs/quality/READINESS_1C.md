...
мастер-планом и падает, если критерий остался без строки, статус `тест`
указывает на несуществующий файл, а `не выполнено` — на незаписанный дефект.
Без этой проверки такая таблица через месяц описывает прошлое.
...
| 3 | Снятие ядра capability невозможно | тест | `scripts/test-capability-artifacts.py` |
...
| 10 | Required/conditional/optional разделены и названы в документации | тест | `scripts/test-1c-readiness.py` |
...
| 16 | В templates, scripts и docs нет секретов и машинных путей | тест | `scripts/test-no-secrets.py` |
| 17 | Плагин и server-vs-client guard требуются только для `ordinary` | не выполнено | №173 |
| 18 | На не-Windows работают bootstrap, Git, документация и валидаторы | тест | `scripts/test-bootstrap.sh` |
...
| 24 | Companion-файлы создаются как seed и видны обоим клиентам | тест | `scripts/test-one-c-scaffold.py` |
...
| 27 | Внешние ссылки каталога проверяются report-only | тест | `scripts/test-1c-readiness.py` |
| 28 | Release блокируется без принятой практики в `practices/1c` | не выполнено | №174 |
| 29 | Реестр несёт `support_mode` и `source_format`; версия БСП записана | не выполнено | №175 |
| 30 | `deploy-and-test-1c` возвращает отчёт YAxUnit | не выполнено | №172 |
| 31 | Отказы «вне scope v1» названы в пользовательской документации | тест | `scripts/test-1c-readiness.py` |

## Итог

Из 31 критерия проверяются тестом 13, ждут Windows-сессии 3, не выполнены 15.

Пятнадцать невыполненных — это не пятнадцать разных проблем, а четыре причины:

- **№172 — upstream capability недоступен** (критерии 5, 22, 25, 30). Пока
  нечего импортировать, ledger пуст, 13 команд не адаптированы, OpenSpec не
  поставлен, YAxUnit нечем запускать. Выдумать содержимое вместо upstream —
  значит завести второй канон поведения.
- **№173 — у гейтов live-base нет машинного исполнителя** (критерии 8, 9, 11,
  12, 13, 14, 17, 20). Правила записаны в skills и в scoped `AGENTS.md`, но
  «проверяется текстом skill» и «проверяется тестом» — разные вещи, и приёмка
  требует второго.
- **№174 — гейт release по принятой практике не реализован** (критерий 28).
- **№175 — версия БСП нигде не хранится** (критерий 29). Реестр баз проверяется
  тестом, а карточки `PROJECT_1C.md` из решения 1.20 в поставке нет.

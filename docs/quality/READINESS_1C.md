...
Из 31 критерия: `тест` — 29, `частично` — 0, `Windows` — 0, `не выполнено` — 1, `отложено` — 1.
Счёт сверяется с таблицей машинно: цифра в итоге — это то место, где такой
документ врёт первым.

Остался один невыполненный критерий:

- **№199 — нет разбора отчёта YAxUnit** (критерий 30). Загрузку исходников
  выполняют доставленные скрипты `db-load-*.ps1` (№165 закрыт), но прогон
  тестов исполнителя не имеет: YAxUnit — условная зависимость, которую

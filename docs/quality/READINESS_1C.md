...
Из 31 критерия: `тест` — 19, `частично` — 4, `Windows` — 0, `не выполнено` — 7, `отложено` — 1.
...
- **№173 — часть диагностики и гейтов без исполнителя** (критерии 10, 11, 12,
  15, 17, 19, 20). Гейт живой базы (`scripts/one_c_session.py`) и ядро
  диагностики (`scripts/one_c_doctor.py`) исполнителей получили: критерии 8, 9,
  13 и 14 закрыты тестом. Остались обнаружение Docker provider, версии EDT и
  профилей, prompt каталога компонентов, guard плагина и backup перед записью —
  всё это требует runtime 1С, а не текста. Правила записаны в skills и в scoped

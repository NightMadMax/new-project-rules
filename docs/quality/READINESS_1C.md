...
- **№197 — нет `develop-1c` и command-owner workflows** (критерий 22
  наполовину). Доставка выполнена (№172 закрыт, PR #46): 13 ролей стоят в обоих
  клиентах и проверяются тестом, OpenSpec с четырьмя workflow закрыл критерий 25.
  Тринадцати команд нет: `evolve-1c-rules` и `manage-1c-mcp` отсутствуют,
  `deploy-1c-source` и `deploy-and-test-1c` объявлены без исполнителя (№165).

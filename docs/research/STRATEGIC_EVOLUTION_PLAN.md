...
к отдельно выбранному legacy repository.

Следующий implementation batch — Этап F, supply-chain hardening:

1. pin GitHub Actions по immutable commit SHA и добавить Dependabot;
2. добавить CI-проверку mutable action references;
3. оформить bootstrap/global-policy threat model;
4. добавить macOS smoke workflow с ограниченным trigger;
5. отдельно согласовать и применить GitHub ruleset как внешнее действие.

...
|---|---|---|
| A. ADR и contract skeleton | completed | `STANDARD_VERSION=2`, последовательный graph `0 → 1 → 2`, TSV fixtures и parity tests |
...
| F. Supply-chain hardening | completed | SHA pins, enforced Actions policy, Dependabot, threat model, macOS smoke и active ruleset `Protect main` (`id: 18603924`) |
...
Файл содержит только происхождение bootstrap и не хранит локальные пути или
изменяемое описание проекта.
...
3. Применить GitHub ruleset. — completed; active ruleset `Protect main`
   требует PR, approval, Code Owner review, resolved threads и strict
   `shell`/`powershell`, запрещает deletion и non-fast-forward; API evidence
   записан в [[ACTIONS]].
...
смешивать с adoption rate отдельных Best Practices.

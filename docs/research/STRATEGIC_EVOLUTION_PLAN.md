...
| D. Global sync | read-only completed | managed-block grammar, check/diff, doctor и CI; apply отложен до migration infrastructure |
| E. Migration engine | plan-only completed | manifest, metadata schema, project/global plans и preconditions; apply ещё отсутствует |
...
4. Отдельно реализовать atomic backup/write и только затем разрешить `--apply`.

Gate: все изменения представлены как reviewable git diff.

### Этап F. Supply-chain hardening

1. Pin Actions, добавить Dependabot и threat model.
2. Добавить macOS smoke workflow.
3. После разрешения применить GitHub ruleset.
...
Этапы A–D и plan-only часть Этапа E завершены без auto-fix. Активный global
file не мигрирован и ожидаемо определяется как `legacy_exact`.

Следующий implementation batch — write path Этапа E:

1. повторно проверять plan fingerprint и preconditions непосредственно перед
   записью;
2. для global policy создавать timestamped backup и выполнять atomic replace;
3. для project metadata требовать clean tree, записывать один файл и оставлять
   reviewable unstaged git diff;
4. добавить explicit confirmation, idempotence и interrupted-write tests;
5. только после совместного review применить marker adoption на текущем
   компьютере и metadata adoption на выбранном pilot project.

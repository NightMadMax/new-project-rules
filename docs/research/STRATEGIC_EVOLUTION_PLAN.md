...
Этапы A и B завершены без изменения состава создаваемых профилей. Следующий
безопасный batch — read-only часть Этапа C:

1. определить validator schema и стабильные exit codes;
2. реализовать проверки required core, profile tree и metadata readiness;
3. добавить проверки wikilinks, placeholders, local paths и raw memory;
4. подключить validator к CI сначала в report-only development mode;
5. не добавлять auto-fix до стабилизации false-positive rate.

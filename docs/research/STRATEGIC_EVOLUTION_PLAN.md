...
## Предлагаемый первый implementation batch

После согласования плана начать только с Этапа A:

1. ADR по TSV + hybrid runtime.
2. `STANDARD_VERSION=1`.
3. read-only contract fixtures текущих четырёх профилей.
4. tests, сравнивающие fixtures с обоими bootstrap outputs.

Этот batch не меняет структуру создаваемых проектов и даёт безопасную точку
остановки перед рефакторингом bootstrap.

...
1. Реализовать read-only legacy inspection и migration plan.
2. Добавить первую миграцию metadata adoption.
3. Ввести clean-tree guard и idempotence tests.
4. Только затем разрешить `--apply`.
...
Этапы A–C и read-only часть Этапа D завершены без auto-fix. Активный global
file не мигрирован и ожидаемо определяется как `legacy_exact`.

Следующий безопасный batch — планирующая часть Этапа E:

1. определить общий migration manifest: ID, from/to schema, preconditions;
2. реализовать plan-only inspection для legacy projects и global marker
   adoption;
3. добавить clean-tree guard, reviewable diff, backup contract и idempotence
   tests до появления любой записи;
4. спроектировать первую metadata migration для `.project-standard.json`;
5. разрешить `--apply` только отдельным шагом после совместного review планов.

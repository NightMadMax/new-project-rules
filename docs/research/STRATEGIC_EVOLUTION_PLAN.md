...
```text
<!-- new-project-rules:begin version=1 -->
...
3. Добавить backup и подтверждаемый `apply`.
4. Выполнить отдельную миграцию текущего компьютера.
...
Этапы A–C завершены без auto-fix и без изменения состава создаваемых профилей.
Следующий безопасный batch — read-only часть Этапа D:

1. определить managed-block grammar и конфликтные состояния;
2. реализовать `sync-agents --check` и secret-safe `--diff` без записи;
3. добавить fixtures для missing, matching, drifted и malformed blocks;
4. отдельно спроектировать подтверждаемый `--apply` с backup;
5. не мигрировать активный global file до review read-only результатов.

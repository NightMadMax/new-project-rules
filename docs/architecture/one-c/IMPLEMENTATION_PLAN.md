...
- Один этап — одна ветка `feat/1c-<этап>`; в `main` попадает завершённый этап с
  зелёными проверками, а не промежуточное состояние. Вливание — обычным pull
  request с прохождением required checks: admin-bypass ruleset `Protect main`
  остаётся исключением только для выпуска release capability (решение 1.25) и на
  код этапов не распространяется.

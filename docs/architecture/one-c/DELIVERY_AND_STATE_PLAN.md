...
**Статус — черновик.** Ни одна правка скриптов, манифестов и схемы не
выполняется до итогового утверждения мастер-плана: гейт «Режима проработки»
действует и здесь. Разделы ниже фиксируют контракт, а не готовый код.
...
| `binary` | EPF, `.launch` и другие небайтовые артефакты | Копирование в бинарном режиме, никаких текстовых преобразований |
...
- Хранить `release_id` capability в `.project-standard.json` или только в
  ledger. Предложение: в metadata — краткая запись, в ledger — hashes.
- Нужен ли отдельный `payload_class` для `.launch`-шаблонов: формально это
  текст с плейсхолдерами, то есть `template`, но содержимое машинозависимо.
- Порог, после которого vendored subtree проверяется выборочно, а не пофайлово
  (91 файл на skill — приемлемо; при росте состава это станет заметно).

...
- У ledger собственное поле `schema_version` (как в metadata), и его изменение — такая же строка в
...
Реализовано этапом Э1. Контракт, уточнённый ревью этапа:

- реестр `config/skills.tsv` — единственный драйвер обхода, но не единственный
  источник истины: фактические каталоги skills и строки `config/capabilities.tsv`
  сверяются с ним, поэтому забытая строка — ошибка, а не тихий пропуск;
- список hash побайтного subtree живёт в `config/skills-payload/<skill>.sha256`,
  а не внутри subtree: манифест рядом с охраняемыми файлами правится тем же
  движением, что и они;
- пути в payload-манифесте обязаны оставаться внутри subtree; симлинки внутри
  vendored subtree запрещены;
- перед первым vendored skill его subtree должен получить `-text` в
  `.gitattributes`: нормализация переводов строк на Windows изменит байты и
  сломает hash.

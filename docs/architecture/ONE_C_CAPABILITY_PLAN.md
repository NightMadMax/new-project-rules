...
1. Перед вызовом XML-зависимого инструмента канон конвертируется в XML во
   временный **gitignored** каталог. Он не является вторым источником истины и
   не переживает операцию.
...
- Стек Best Practices `1c` содержит хотя бы одну принятую практику с evidence:
  без этого первый release capability не выпускается.

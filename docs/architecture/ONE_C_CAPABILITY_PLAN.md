...
| Платформа 1С | обязательна | версия ИБ (может отличаться по контурам) | runtime базы |
| Java (для EDT) | обязательна | требуемая EDT | запуск EDT |
| Отредактированный MCP EDT | обязателен | совместим с версией платформы/EDT | см. «Настройка EDT» |
| Docker/WSL | рекомендуется | — | search-серверы (Syntax/Help/SSL/Templates) |
...
| 1С:Напарник (CodeChecker) | опционально | — | отдельная подписка, секреты вне репозитория |

- **Обязательный минимум:** Windows + EDT(+плагин) + платформа + Java +
  отредактированный MCP EDT.
- Docker и search-серверы дают чтение справки/БСП/синтаксиса; без них работа
  возможна с деградацией удобства, но не безопасности.
...
### Проверенный опорный стек

Фактически проверено на машине разработки (2026-07-20). Фиксируются **версии**,
не пути: конкретные пути машинные и в шаблоны не попадают.

| Компонент | Проверено | Роль |
|---|---|---|
| 1C:EDT | `2026.1.2+2` | обязательный |
| JDK из поставки EDT | `axiom-jdk-full 17.0.16+12` | обязательный (идёт с EDT) |
| Платформа 1С | `8.3.24.1819` (также `8.3.24.1342`, `8.3.27.1936`) | обязательная |
| Docker Desktop / WSL | `29.6.1` / WSL `2` | рекомендуется (search-серверы) |
| PowerShell | `7.6.3` | сборщик EPF, preflight |
| Python | `3.12.10` (мин. 3.10) | сборщик `pack_form_bin.py`, скрипты |
| git | `2.54.0` | версионирование |
| Плагин обычного приложения | `24.02` + патч EDT 2026 | опционально (legacy) |
| uv | `0.11.24` | опционально |
| Node.js | `v24.18.0` | опционально |

Системная Java может быть старой (на машине `1.8.0_491`) — это не мешает: EDT
использует свой bundled JDK 17. `verify-1c-workspace` проверяет наличие и версии
**без установки** и сообщает, чего именно не хватает.

...
|---|---|---|
| Операция ушла не в ту базу: порт постоянный, база за ним меняется | Подтверждение фактической базы реальным MCP-вызовом | 3 |
...
3. **Один раз за сессию** `verify`/`select` подтверждают фактическую базу за
   ...
   переключателя записи). Порт `6003` не меняется, а база за ним — да; это
   единственная защита от «ушёл не в ту базу». Подтверждение **аннулируется** и

...
| 1.14. Bootstrap, setup и классы зависимостей | согласовано 2026-07-25; interaction contract дополнен | Пользовательский workflow един, но имеет две границы отказа: `create-new-project` транзакционно создаёт только репозиторий, затем сразу предлагает `setup-1c-environment`, который через read-only `doctor-1c` строит machine-change plan и после явного approval применяет разрешённые изменения. Каждый компонент рассматривается отдельно: setup объясняет простыми словами, зачем он нужен, какие функции включает и что перестанет работать при отказе; всегда показывает официальный источник, версию, admin/restart impact и предлагает `установить автоматически`/`установить самостоятельно`/`пропустить`. Найденный компонент также требует выбора `использовать`/`обновить`/`не использовать`. Результат фактически перепроверяется. Зависимости делятся на `required`, `conditional` и `optional`; Node.js, `docx`, Pillow, `v8unpack` и OpenSpec CLI — conditional feature dependencies. |
...
Runtime-операции capability `1c` — **только для Windows**. EDT, платформа 1С,
встроенный Toolkit и отладка предполагают Windows-хост; запуск этих операций
на macOS/Linux не поддерживается. Bootstrap, Git, документация, review и
валидаторы стандарта остаются кросс-платформенными.

`doctor-1c` — источник истины по фактическому наличию и версиям;
...
- На **не-Windows** `doctor-1c` помечает Windows runtime как недоступный и
  блокирует 1С-операции. При этом он продолжает доступные проверки Git,
  документации, release artifacts и конфигурации; валидаторы стандарта работают
  штатно.
...
- не переустанавливает найденные компоненты без отдельного выбора;
...
Каждый компонент показывается отдельным интерактивным шагом. До вопроса setup
обязан объяснить:

- для чего компонент нужен в 1С-workflow;
- какие skills/MCP/операции он включает;
- является ли он required, conditional или optional;
- что именно останется недоступным при пропуске;
- рекомендуемую/обнаруженную версию;
- стабильную официальную страницу загрузки и документацию;
- возможные admin, restart, network и credential последствия.

Для отсутствующего компонента предлагаются `установить автоматически`,
`установить самостоятельно` или `пропустить`; для найденного —
`использовать`, `обновить/переустановить` или `не использовать`. Перед
автоматической установкой показываются точная команда, источник, версия и
область изменений. При ручном варианте workflow выводит/открывает официальный
источник, ждёт продолжения и затем проверяет факт установки, а не доверяет
только текстовому подтверждению.

Пропуск required даёт итог `incomplete`; пропуск conditional перечисляет
отключённые функции; пропуск optional не ухудшает основной статус. Если
официальный источник требует входа, пользователь проходит его самостоятельно.
Пароль, token или license key setup не читает и не сохраняет.

`config/1c-release.json` хранит для каждого компонента `class`, понятные
`purpose`/`enables`/`missing_effect`, `version_policy`,
`official_download_page`, `official_documentation`, `install_mode`
(`automatic`/`assisted`/`manual`), `admin_required`, `restart_policy` и
`verification_command`. Временный binary URL в manifest не pin-ится: setup
разрешает его из стабильной официальной страницы. Weekly release check
проверяет доступность канонических страниц; неизвестное зеркало без отдельного
...
Каждый шаг — по запросу пользователя; `doctor-1c` подтверждает результат
...
- `doctor-1c` проверяет наличие профилей и `ATTR_CLIENT_TYPE`.
...
1. `doctor-1c` — проверяет EDT, платформу, Java, Docker/WSL,
   обязательный plugin, patch state, профили, свободные порты, все поставляемые
   обработки Toolkit по SHA-256 и внешний MCP provider без установки или
   изменения состояния.
...
4. Собрать полный проектный каркас capability: артефакты из `1C/`, реестры и
...
7. Добавить regression tests: одиночная база, несколько баз на разных портах,
   ...
   наличие обязательных компонентов из «Требований к среде», shell/PowerShell parity скриптов
...
- Для каждого компонента setup до выбора объясняет назначение, включаемые
  функции и последствия отказа, показывает официальный download/docs source,
  версию и admin/restart impact, предлагает автоматическую/ручную
  установку/пропуск и затем фактически перепроверяет результат.

...
Отложенное и явно названное: восемь дефектов механики доставки (№122–№129),
контракт которых зафиксирован в подплане, а код ещё не написан. Пересборка
обработки Toolkit для `managed` из этого списка убрана — решение 1.16
пересмотрено 2026-08-02, пересобирать ничего не нужно. Дефекты №61, №137 и
№166 закрыты вехой W 2026-07-30.
...
| 1.16. Профиль запуска для `managed` | направление принято 2026-07-26; пересмотрено 2026-08-02 | Профиль обязателен для обеих ветвей `application_kind` и поставляется для обеих. Разведены не профили, а атрибуты: `ATTR_CLIENT_TYPE` и read-only EPF применяются только к `ordinary`, потому что только там можно стартовать не тем клиентом и только там барьер записи стоит в самом артефакте. Пересборка обработки **отменена**: берутся уже собранные. Их три, все для обычного приложения — read-only, read-write и privileged, — и профилей поэтому тоже три, различающихся ровно одной строкой `ATTR_STARTUP_OPTION`. Плюс профиль HTTP-debug без `/Execute`. Решение 1.6 остаётся в силе: сборщик EPF не поставляется, capability отдаёт готовые файлы. Для `managed` собственной обработки в проекте нет — там барьер записи не в артефакте, а в переключателе UI Toolkit, и подтверждается он вызовом (остаток критерия 7, дефект №240). `doctor-1c` проверяет `ATTR_CLIENT_TYPE` и наличие автозапуска у профилей `ordinary`. Закрывает дефект №130. |
...
| 1.31. Классы разрешений без объявленных инструментов | согласовано 2026-07-27; уточняет 1.17 | Пока каталог ролей не объявляет `tools`, сервер получает строгий класс своей области целиком: EDT и Toolkit — `ask`, включая чтение. Имена инструментов знает только upstream, а придуманные правила не совпали бы ни с чем и создали бы видимость точности. Точность включается автоматически при появлении `tools` в каталоге. Серверное правило при этом не выпускается: Claude Code разрешает `deny → ask → allow`, и широкое `ask` перекрыло бы разрешённые инструменты. Цена решения — подтверждение на каждом чтении EDT до импорта upstream. |
| 1.33. Каталог сужает, но не расширяет | согласовано 2026-07-27; уточняет 1.31 | У роли две величины: fallback — что получает сервер целиком, пока инструменты не объявлены, и ceiling — максимум, который каталог вправе выдать объявленному инструменту. Каталог ролей редактируется в проекте, поэтому без потолка строка `tools` снимала бы единственный `deny` политики. Роль `data` имеет ceiling `deny` и не объявляет инструменты вовсе: перечислить их значило бы создать способы до неё дотянуться. |
| 1.32. Проекции вне ledger | согласовано 2026-07-27; уточняет S.4.2 | Клиентские проекции детерминированно пересобираются из каталога ролей и реестра баз, поэтому дрейф внутри owned-блока не отслеживается ledger'ом: источник истины всегда доступен, а «конфликт» здесь означал бы остановку на файле, который и так будет пересобран. Транзакционность сохраняется на уровне записи: все три файла пишутся через staging и переименование, частично применённой проекции не бывает. |
...
| 1.25. Маршрут выпуска release | согласовано 2026-07-26; уточнено 2026-07-30 | Release capability фиксируется прямым commit и push в `main`, но это признаётся **admin-bypass** активного ruleset `Protect main`, а не обычным путём. Каждый такой выпуск сопровождается записью в `ACTIONS.md` по образцу уже имеющихся записей: что выпущено, почему обошли PR, какие проверки прогнаны локально. Draft PR по-прежнему не используется. Закрывает дефект №136. Уточнение 2026-07-30: код этапов вливается **обычным** pull request без bypass. Раньше это было невыполнимо — ruleset требовал approval, которое единственный владелец не может выдать себе, поэтому все этапы шли через `--admin` (дефект №190). Требование approval снято с ruleset, обязательными остались PR и зелёные required checks; `--admin` вернулся к роли исключения. |
...
- `per-base` — Toolkit и другие live-base servers на пару
...
`per-base` **разделяет не порт, а канал**. Toolkit-обработка не поднимает
собственный HTTP-сервер: на `127.0.0.1:6003` слушает один прокси, а базы
разводятся параметром `?channel=<канал>` в URL. Обработка подключается к прокси
клиентом и объявляет тот же канал. Пересмотрено 2026-08-18; прежняя схема
«отдельный встроенный сервер на базу, порт из диапазона `6003`–`6012`»
зафиксирована в дефекте 83 и больше не действует.

Следствия, которые надо держать в голове:

- канал обязан совпадать в двух местах — в URL клиента MCP и в параметрах
  запуска обработки; расхождение даёт таймаут, а не внятную ошибку;
- одновременно обслуживается несколько баз, чего прежняя схема не давала:
  порт занимала одна обработка;
- статус `Connected` у MCP-сервера подтверждает живость прокси, а не наличие
  клиента 1С за каналом.
...
| 1C MCP Toolkit (через прокси) | generated `onec-toolkit-*` | initial | Данные, метаданные и операции живой базы | write-capable (см. ниже) | Прокси слушает `127.0.0.1:6003`; на выбранной базе запущен runtime-клиент с обработкой Toolkit, объявившей тот же канал |
...
shared Git не записываются. Собственный порт `6003` принадлежит
...
**Сборщик EPF не поставляется** — и пересматривать это не потребовалось:
решение 1.16 отменило пересборку, а обработки уже собраны. Для управляемого
...
- **Что допустимо индексировать.** Только обезличенные шаблоны кода/запросов,
  проверенные паттерны и ссылки на первоисточники.
...
- **До импорта upstream чтение EDT спрашивает.** Каталог ролей не объявляет
  инструменты, поэтому EDT и Toolkit получают `ask` целиком (решение 1.31).
  Основной рабочий маршрут требует подтверждения на каждом вызове — это цена
  отказа выдумывать имена инструментов.
- **Гейты skills не доходят до Claude Code.** Мосты `.claude/skills` поставляются
  вместе с agent-проекциями после импорта upstream, поэтому capability-skills
  видит только Codex. Для Claude Code правила 1, 2, 5, 7 и 8 подняты в scoped
  `AGENTS.md`, но first-барьер там — текст правил, а не отдельный исполнитель.
- **Роль `data` нельзя включить средствами проекта.** Каталог ролей —
  managed-артефакт, признака «включено после security review» в поставке нет,
  и включение вручную снимается следующей сборкой проекций. До появления
  носителя признака включение требует изменения стандарта.
...
|---|---|
| CI/CD-пайплайн на Windows-раннере | Не хватает **только раннера**: загрузка (`db-load-*`), запуск базы (`db-run`), YAxUnit и разбор его отчёта (`one_c_yaxunit.py`) уже поставляются и проверяются тестами. Осталась инфраструктура и решения вокруг неё — где живёт self-hosted раннер, кто его обновляет, лицензия платформы на нём. Уточнено 2026-08-02: прежняя формулировка «требует отдельного подплана» устарела, когда исполнители появились |
| Миграция из хранилища конфигурации в Git | Разовая операция под конкретную базу: объём, авторы и соответствие пользователей хранилища авторам Git выводятся из этой базы, а не из стандарта. После переноса инструмент проекту больше не нужен. Стандарт поставляет повторяющееся; это — рецепт для Best Practices |
| SonarQube, АПК | Дублируют то, что уже выполняется. BSL Language Server, который поставляется в v1, и есть анализатор, на котором построен SonarQube-плагин для BSL: диагностики те же, добавились бы сервер и витрина, а не проверки. Правила АПК пересекаются со стандартом ИТС, закрытым гейтом `review_1c_code`. Уточнено 2026-08-02: прежнее «избыточен для внутренних доработок» было оценкой контекста, а не свойством инструментов |
| Обновление типовой через `cfu` | Автоматизируется команда, но не решение. Конфигуратор применяет `.cfu` в пакетном режиме; неавтоматизируема цепочка: какие релизы проходить последовательно, что делать с конфликтами режима поддержки, когда снимать замок с объектов. Уточнено 2026-08-02: прежнее «не автоматизируется» читалось как «нельзя», хотя механический шаг возможен и остаётся за человеком только выбор |
...
- Выборки данных базы **не запрещены** (решение по разделу «Безопасность»):
...
2. **Toolkit через общий прокси.** Capability резервирует один порт
   **`6003`** под прокси; базы разводятся каналами, число одновременно
   экспонируемых баз не ограничено (пересмотрено 2026-08-18).

- Для строки с `mcp_enabled=true` `toolkit_channel` обязателен и уникален среди
  остальных включённых строк. Renderer публикует только такие строки в
  Codex/Claude client projections.
- Для строки с `mcp_enabled=false` `toolkit_channel` пуст и канал не
  резервируется.
...
3. Если `mcp_enabled=true`, задать `toolkit_channel`, не занятый другой
   включённой строкой. Если `mcp_enabled=false`, оставить его пустым.
   Доступность прокси и уникальность каналов проверяет `doctor-1c` без
   автоматического переназначения.
...
|---|---|---|
| `Запуск Toolkit` | Анализ данных живой базы | `ATTR_STARTUP_OPTION=/Execute "<путь-к-EPF>" /C "startup;mode=proxy;url=…;channel=…;write=0;privileged=0"` — автозапуск обработки и выбор режима |
...
- **Сборка одна, режим задаётся параметром запуска** (пересмотрено 2026-08-18;
  прежде было три взаимоисключающие сборки). `/Execute` указывает на единственный
  EPF, а `/C "startup;…"` задаёт `mode`, `url`, `channel`, `write` и `privileged`.
  Профиль `Запуск Toolkit` ставит `write=0`; запуск с записью или привилегиями
  идёт через отдельный профиль под подтверждённую задачу.
- `write=0` **не является запретом записи**: флаг отключает только
  авторазрешение операций, которые прокси пометил `requires_approval`. Реальный
  набор разрешённых операций ограничивается на стороне прокси. Проверено
  2026-08-18: при `write=0` запись документа через `execute_code` прошла.
- Если EDT-плагин обычного приложения не пропускает `/C` в
  `ATTR_STARTUP_OPTION`, профиль запускается с умолчаниями сборки, а режим
  задаётся ярлыком командной строки. Это проверяется при инстанцировании
  профиля, а не предполагается.
...
```text
project_id	environment_id	folder	configuration	platform_version	compatibility_mode	application_kind	support_mode	source_format	edt_workspace	edt_profile	toolkit_channel	is_production	mcp_enabled	owner
```
...
| `toolkit_channel` | Канал базы на прокси Toolkit | Для `mcp_enabled=true`: обязателен, 1–64 символов `a-zA-Z0-9_-`, уникален среди включённых строк; для `mcp_enabled=false` пуст |
...
7. Добавить regression tests: одиночная база, несколько баз на разных портах,
   ...
   некорректном канале, уникальность среди включённых строк и блокировка
...
Что из перечисленного ниже действительно проверяется и чем — в
[[docs/quality/READINESS_1C|матрице готовности]]; она сверяется с этим разделом
машинно.
...
- В общей рабочей области число зарегистрированных баз не ограничено портами;
  строки с `mcp_enabled=true` разведены по каналам, а локальный

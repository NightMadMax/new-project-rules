...
2. 1С:Напарник: плагин из репозитория `https://code.1c.ai/plugin/` (раздел `AI`)
   ...
   и ключ».
3. *(legacy)* Плагин обычного приложения.
4. Отредактированный MCP EDT.
5. *(legacy)* Патч `Run without update` под совместимую версию MCP EDT.
...
| Патч `Run without update` слетел после обновления EDT | Запуск без обновления ИБ на базах обычного приложения | Управляемое приложение, разработка, Toolkit |

**SHA-256 EPF — исключение, потому что это граница безопасности, а не версия.**
Только он отличает read-only сборку от write-enabled. Если хеш не совпал ни с
одной из двух известных, неизвестно, может ли запущенная обработка писать в базу,
поэтому блокируется весь Toolkit. Остальные домены деградируют точечно.
...
1. Проверить состав MCP (каталог из семи серверов),
   ...
   (см. «Политика хранения project memory»).

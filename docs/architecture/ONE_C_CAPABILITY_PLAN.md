...
4. Основной план держать картой решений. Детальную проработку крупных блоков
   выносить в связанные подпланы и возвращать сюда краткий статус.
5. После разбора всех разделов провести целостную проверку противоречий,
   полноты, безопасности и проверяемости.
6. Показать итоговую редакцию пользователю и получить явное утверждение всего
   плана.
7. Только после этого сменить `status` на `accepted` и составить план
   реализации.
...
| Toolkit: чтение | `allow` | Разрешено только при подтверждённом session lock; правило 3 остаётся единственной защитой от «ушёл не в ту базу» |
...
- короткая модель ветвления и ревью: ветка на задачу, что именно ревьюится
  (BSL и метаданные), как разрешаются конфликты выгрузки.
...
1. Собрать идентичность базы: уникальный `project_id`, папку
   ...
   совместимости, `support_mode`, **`application_kind`** (`ordinary`/`managed` — от него зависят
...
- `.gitattributes` — managed-блок capability: `*.epf`, `*.erf`, `*.cf`, `*.cfe`
  ...
  обработки и их SHA-256), для `*.bsl`, `*.xml` и `*.mdo` фиксируются eol и
  кодировка. Конкретные text-правила дерева исходников зависят от открытого
  решения 1.21 о формате выгрузки.
...
```text
project_id	environment_id	folder	configuration	platform_version	compatibility_mode	application_kind	support_mode	edt_workspace	edt_profile	server_port	is_production	mcp_enabled	owner
```
...
- Capability не предполагает обычное приложение по умолчанию: conditional
  ordinary-plugin нужен только базам `application_kind=ordinary`.
...
- Настройка EDT (EDT 2026.x, EDT-MCP, conditional-патчи и профили) описана как

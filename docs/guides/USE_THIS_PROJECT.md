...
## Возможности поверх профиля

Профиль задаёт состав документации, а возможность добавляется поверх него и с
любым стеком. Их три, и каждая просится по имени при создании проекта:

- `jira-confluence` — модель Jira и Confluence, MCP-runbook, аналитика и
  трассировка;
- `1c` — разработка на платформе 1С, профиль не ниже `operated`;
- `transcribe` — локальная расшифровка аудио и видео, профиль не ниже
  `software`, включается явно.

> Создай новый проект по стандарту, профиль `operated`, возможность `transcribe`.

Минимальный профиль каждой возможности объявлен в `config/capability-core.tsv`;
попытка подключить её на профиле ниже отклоняется с объяснением.

## Проекты 1С: чего в первой версии нет

Проект `1c` создаётся так же, как любой другой: «создай новый проект по
стандарту с preset `1c`». Дальше стоит знать границы — не потому что это трудно
добавить, а потому что этого сейчас нет, и агент честно откажется:

- **CI/CD для конфигураций 1С.** Готового пайплайна нет, но не с пустого места:
  загрузка исходников в базу и запуск в пакетном режиме выполняются
  vendored-скриптами upstream (`1c-metadata-manage`, каталог `1c-db-ops`) — они
  поставляются и сверяются по хэшу, поведенческих тестов у них нет по природе
  vendored-поставки. Наш здесь только разбор отчёта YAxUnit
  (`scripts/one_c_yaxunit.py`, покрыт `scripts/test-1c-yaxunit.py`): сам прогон
  он не запускает. Не хватает и Windows-раннера — всё перечисленное требует
  платформы 1С и PowerShell. Развёртывание на тестовый контур пока запускает
  человек, теми же скриптами.
- **Миграция из хранилища конфигурации.** Перенос истории из хранилища в Git не
  поддерживается: проект начинается с текущего состояния. Это разовая операция
  под конкретную базу — её объём, авторы и соответствие пользователей хранилища
  авторам Git берутся из этой базы, а после переноса инструмент проекту больше
  не нужен.
- **SonarQube.** Интеграция с ним не поставляется; проверки кода идут через BSL
  Language Server и 1С:Напарник.
- **АПК (автоматизированная проверка конфигураций).** Прогон не автоматизирован.
- **Обновление типовой через `cfu`.** Файлы обновления не применяются
  автоматически; обновление типовой остаётся ручной операцией.

Что именно уже проверяется тестом, что ждёт Windows-сессии, а что не сделано —
в [[docs/quality/READINESS_1C|матрице готовности]].

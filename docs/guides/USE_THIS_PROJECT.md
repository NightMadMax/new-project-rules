...
- **CI/CD для конфигураций 1С.** Готового пайплайна нет, но не потому, что
  нечем: загрузка исходников в базу, запуск базы в пакетном режиме, прогон
  YAxUnit и разбор его отчёта уже поставляются и проверяются тестами. Не хватает
  Windows-раннера — всё перечисленное требует платформы 1С и PowerShell, на
  ubuntu-раннере не запустится. Развёртывание на тестовый контур пока запускает

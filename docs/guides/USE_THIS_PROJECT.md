...
### 11. Описать процесс и держать описание живым

Codex: `$document-process-workflow`

Claude Code: `/document-process-workflow`

Фразы:

> Опиши процесс работы с практиками отчётом: полный процесс и отдельно
> workflow пользователя.

> Скилл изменился — приведи workflow-отчёты в соответствие.

Когда использовать:

- появился процесс, который выполняют несколько ролей;
- отчёт разошёлся с фактическими скиллами, скриптами или структурой.

Отчёты живут в `docs/guides/user-guide/workflows/`.

### 12. Расшифровать запись встречи

Требуется проект с capability `transcribe`.
...
> Расшифруй запись встречи и подготовь протокол.

> Разбери демонстрацию интерфейса по кадрам.

Когда использовать:

- есть запись встречи, интервью или демонстрации;
- нужен текст, по которому дальше работает человек или агент.

Расшифровка идёт локально: медиа не уходит во внешний сервис. Наружу попадает
только запрошенное — готовая расшифровка для протокола и кадры для разбора
интерфейса. `faster-whisper`, модель и FFmpeg — условные зависимости, они
ставятся с разрешения.
...
- **CI/CD для конфигураций 1С.** Сборка и публикация из репозитория не
  автоматизированы; развёртывание на тестовый контур выполняется человеком.
- **Миграция из хранилища конфигурации.** Перенос истории из хранилища в Git не
  поддерживается: проект начинается с текущего состояния.

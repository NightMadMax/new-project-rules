...
Маршруты upstream проверяются отдельно, по локальному чекауту закреплённого
commit:

```sh
python3 scripts/import_1c_upstream.py --staging ../ai_rules_1c
```

Команда только считает и печатает: сколько файлов, сколько строк ledger, сколько
строк ещё ждут генератора. С `--write` она записывает ledger, но отказывается,
пока хоть одна строка объявляет выход, которого не существует. Файл без маршрута и маршрут без файла — ошибки:
первое означает, что артефакт молча выпал из поставки, второе — что решение
описывает то, чего в upstream уже нет. Сами маршруты живут в
`config/1c-routing.tsv` со ссылкой на решение плана, а список файлов на
закреплённом commit — в `config/1c-upstream-inventory.txt`; он позволяет
проверять покрытие в CI без клона.

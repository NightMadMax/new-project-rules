...
Если нужно сразу проверить оба агента, укажите режим `both`. Если компьютер
будет использовать только Claude Code, укажите `claude`.

Агент должен:

1. Найти или клонировать `new-project-rules` внутри общего рабочего vault.
2. Определить `agent mode` и запустить `project-doctor`.
3. Проверить `gh auth status`, Git identity и credential helper.
4. Настроить `~/.codex/AGENTS.md` и `~/.claude/CLAUDE.md` через
   `setup-global-agents`.
5. Показать read-only состояние global policy через `sync-global-agents`.
6. Построить global migration plan и применять его только после явного
   подтверждения fingerprint.
7. Сообщить, что настроено, какие проверки пройдены и что ещё осталось вручную.

Если какого-то инструмента не хватает, агент должен сначала объяснить, что
именно будет установлено и зачем, и получить разрешение до изменения системы.
...
Глобальный файл содержит только правила, применимые почти к любой сессии.
Структура документации проекта, defect log, playbook, knowledge promotion и
другие процессы стандарта остаются в проектных шаблонах и skills, а не
дублируются в глобальном контексте.
...
ещё не содержит managed markers. `unmanaged_conflict` означает, что в файле уже
есть ваши собственные правила без markers — типичный случай, когда на компьютере
раньше велись работы. Оба состояния — корректные точки adoption. `sync` ничего
не записывает; markers нельзя добавлять вручную — их установка выполняется
отдельной подтверждаемой миграцией: для `legacy_exact` она оборачивает текст
markers, для `unmanaged_conflict` дописывает managed block ниже ваших правил,
сохраняя их. Полная модель состояний описана в
[[docs/guides/SYNC_GLOBAL_AGENTS|руководстве по global policy sync]].

Отдельно постройте migration plan с preconditions будущего backup/apply:
...
Planner не изменяет active file. После review и явного подтверждения примените
только тот же fingerprint:
...
Сохраните напечатанный путь backup и убедитесь, что повторный check показывает
`managed_match`. Интерпретация статусов и rollback описаны в
[[docs/guides/PLAN_MIGRATIONS|руководстве по миграциям]].

## 6. Проверить загрузку инструкций

Изменения instruction-файлов применяются к новым сессиям. Поэтому после setup
или migration запустите новый процесс Codex из проекта:

```sh
codex --ask-for-approval never exec "Summarize the current instructions."
```

Для проверки вложенного scope запустите ту же команду с
`codex --cd <directory> --ask-for-approval never exec "Summarize the current instructions."`
и убедитесь, что ближайший `AGENTS.md` имеет приоритет
в своём поддереве.

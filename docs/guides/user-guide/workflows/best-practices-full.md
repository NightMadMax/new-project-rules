...
|---|---|---|---|---|---|
| A1 | Зафиксировать факт | агент·по правилу / пользователь→агент | `auto` (агент сам пишет дефект по правилу AGENTS.md) **или** `skill` — *«Зафиксируй урок»* · `/reflect-and-record` | Сбой/приём из работы → `DEFECTS.md` / `PLAYBOOK.md` (≥2 раз либо проверенное решение после перебора неудачных вариантов) / ADR | внешн.: обнаружен дефект в ходе работы |

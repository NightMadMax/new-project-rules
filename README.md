...
4. Чтобы Claude Code читал те же глобальные правила, связать его файл с файлом
   Codex (один раз на компьютер):

   ```sh
   ln -s ~/.codex/AGENTS.md ~/.claude/CLAUDE.md
   ```

   На Windows, где симлинки требуют прав, вместо ссылки создать
   `~/.claude/CLAUDE.md` из одной строки `@~/.codex/AGENTS.md`.
...
и те же правила без дублирования. Все правила держите в `AGENTS.md`.

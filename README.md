...
Возможность `jira-confluence` подключается поверх любого профиля (например,
`operated` + `jira-confluence`) и добавляет модель Jira/Confluence, MCP-runbook,
аналитику, трассировку и два агентских skill. Это не новый взаимоисключающий
профиль: она совместима с любым стеком, в том числе `1c`.

...
- создаёт новый проект по одному из профилей: `minimal`, `software`,
  `operated`, `all`;
- подключает возможности `jira-confluence`, `1c` и `transcribe` без изменения
  базового профиля;
...
- [[.agents/skills/document-process-workflow/SKILL|document-process-workflow]] —
  описывает процесс отчётом по ролям и поддерживает отчёт в согласии со
  скиллами и скриптами, когда те меняются.
...
Возможности подключаются поверх профиля и не исключают друг друга:

- `jira-confluence` — модель Jira и Confluence, MCP-runbook, аналитика,
  трассировка и два агентских skill;
- `1c` — разработка и эксплуатация 1С: реестр баз, EDT, MCP-провайдер,
  диагностика среды и гейты работы с живой базой;
- `transcribe` — локальная расшифровка встреч и интервью: медиа не уходит во
  внешний сервис.

Актуальный список — колонка `capability` в `config/capabilities.tsv`. Часть
возможностей требует минимального профиля (`config/capability-core.tsv`):
bootstrap поднимает профиль сам, отказа при этом не происходит.

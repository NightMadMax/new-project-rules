...
- создаёт новый проект по одному из профилей: `minimal`, `software`,
  `operated`, `all`;
- подключает независимые возможности, включая `jira-confluence`, без изменения
  базового профиля;
...
Возможности подключаются поверх профиля и не образуют взаимоисключающих
вариантов. Их три:

- `jira-confluence` — модель Jira/Confluence, MCP-runbook, аналитика,
  трассировка и два агентских skill;
- `1c` — разработка на платформе 1С: реестр баз, EDT, MCP-провайдер, скиллы
  разработки и диагностики;
- `transcribe` — локальная расшифровка аудио и видео, включается явно.

Минимальный профиль каждой объявлен в `config/capability-core.tsv`.

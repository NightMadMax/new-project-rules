...
- If a subdirectory needs scoped instructions, create an adjacent
  `AGENTS.md`/`CLAUDE.md` pair. A nearer `AGENTS.md` specializes or overrides
...
- Keep instruction files compact; Codex stops adding files when the combined
  chain reaches `project_doc_max_bytes` (32 KiB by default). Move detail into
  `docs/` or skills.
...
- Record a verified, reusable successful pattern in `docs/quality/PLAYBOOK.md`
  automatically, without waiting for a user reminder, when either it has proven
  correct at least twice or it is the working solution found after testing
  multiple unsuccessful alternatives. In the latter case, record it as soon as
  the successful result is verified so the known-good path replaces the costly
